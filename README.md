...
account from what the app runs under, so the release source dirs are read only
to the app.

It runs the script `/srv/foo/bin/deploy-sync-config-s3` on startup
which pulls app config from S3 in TOML format, putting it in the `/etc/foo` dir.
The `!` at the beginning means the script should be run as root, not as the app user.
...
a temp file to `/run/foo`. It sets `runtime_directory_preserve` to `yes`
to help in debugging startup issues.

```elixir
config :mix_systemd,
  exec_start_pre: [
    # systemd runs this script as root before starting the app
    ["!", :deploy_dir, "/bin/deploy-sync-config-s3"]
  ],
  dirs: [
    # systemd creates /etc/foo
    :configuration, # App configuration, e.g. db passwords, /etc/#{ext_name}
    # systemd creates /run/foo
  ...
  # systemd will not clean up /run/foo dir
  runtime_directory_preserve: "yes",
  env_files: [
    # Optionally read environment vars from file /srv/foo/etc/environment
    ["-", :deploy_dir, "/etc/environment"],
    # Optionally read environment vars from file /etc/foo/environment
...
Here is [a complete example app which uses
mix_deploy](https://github.com/cogini/mix-deploy-example).
...
3. Secrets such as db passwords, API keys and encryption keys.

4. Dynamic information such as the IP address of the server or other
   other machines in the cluster.

Elixir has a couple of mechanisms for storing configuration.
When you compile the release, it converts Elixir-format config files like
`config/config.exs` into an initial application environment that
is read by `Appliction.get_env/3`. That's good for simple, relatively static
apps with no secrets.

Elixir 1.9+ releases run the shell file generated by the template
`rel/env.sh.eex` to set up the environment. They also execute an Elixir file
(`config/releases.exs`) every time the release boots.  You can also run code in
your application startup to pull configuration dynamically.
...
One of the principles of security is "least privlige". If an attacker manages
to compromise your app, then they can do whatever it has permissions to do,
e.g. reading and writing files. If they can get it to remotely execute code,
then that's even worse. Having an executable configuration file that is
writable by the app triggers my paranoia.

Because of this, I prefer to fetch sensitive configuration information outside
the app, and only give the app read only access to it. I prefer to keep
keep configuration files "data only" and easy to valiate.

Beyond that, the configuration process depends on how complex things are.

## Environment vars

The simplest thing is to set environment variables. Add environment vars to
...
env_vars: [
  # Set a variable
  # Best for things that are not sensitive and don't change
  "PORT=8080",
],
dirs: [
  # Ensure creation of /etc/foo
  :configuration,
],
env_files: [
...
`/etc/foo/environment` looks like:
    ...
    DB_USER=foo_prod
    DB_PASS=XXX
    DB_HOST=db-postgresql-sfo2-xxxxx-do-user-yyyyy-0.db.ondigitalocean.com
    DB_PORT=25060

`config/releases.exs` looks like:

```elixir
configuration_dir = System.get_env("CONFIGURATION_DIR")
...
config :foo, FooWeb.Endpoint,
  ...
  https: [
    ...
    cipher_suite: :strong,
    keyfile: Path.join(configuration_dir, "/ssl/https.key"),
    certfile: Path.join(configuration_dir, "/ssl/https.cert.pem"),
    dhfile: Path.join(configuration_dir, "/ssl/https.dh.pem"),
    cacertfile: Path.join(configuration_dir, "/ssl/https.cacert.pem"),
  ],
...
The question is how to get these files onto the server. For simple server
deployents, we can copy the config to the server when doing the initial setup.
...
It's verbose and the data is all simple strings, so you have to encode them
safely and convert them back to lists, integers or atoms.

[Config providers](https://hexdocs.pm/elixir/Config.Provider.html) let you to load
...
We can store config information in an external configuration system and
pull them out at runtime. An example is [AWS Systems Manager Parameter
Store](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-paramstore.html).

While it's possible to pull individual config items, it may be more convenient
to grab all the config data and write it to a file, then read it in with a
Config Provider like [aws_ssm_provider](https://github.com/caredox/aws_ssm_provider).
...
We can write Elixir code to do things like query the system for the primary IP address.
`cloud-init`
[already does this](https://cloudinit.readthedocs.io/en/latest/topics/network-config.html#network-configuration-outputs), we
just have to read the JSON file.
...
This library works fine with Distillery. `exec_start_pre` scripts
are particularly useful in the absense of `rel/env.sh.eex`.

Configure the library to use `:distillery`:
